...
6. 启动后轮询 `/health` 验证进程与数据库；30 秒内仍不可用则自动恢复上一版本二进制，`/health/ready` 的运营告警单独提示
...
更新器以 `/health` 验证新进程和数据库是否能够工作；`/health/ready` 还包含备份时效、外部渠道和运行监控等运营条件。后者未通过时更新器会明确警告但不会把正常启动误判为失败，管理员应在更新完成后进入“运行监控”处理对应告警。
